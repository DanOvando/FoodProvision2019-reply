---
title: "Ommiting Distance from Models of Marine Protected Areas Produces Unreliable Results"
author: 
  - Daniel Ovando
  - Owen Liu
  - Renato Molina
...
Any global model must make compromises to reach an answer. As consumers we must then evaluate those choices made and consider whether they provide on average right results. We feel there are clear signs that this is not the case for this paper. USA Example. Australia to Baja example. China example. Global stock underpinning that. Taking F/Fmsy estimates as unbiased data. 

Moving forward, we should suggest that any model of global MPAs be sufficiently spatially resolved to provide sensible results for XX. 

consider the USA as one MSY weighted stock, suggests upsides is Blah according to their model. 

Following the model as it stands here, there are two truisms that require no simulation: one could close a large percent of the world's oceans to fishign without reducint fishing catch substantially if at all, and if we assume that at least some overlapping combination of stocks are overfished, it is possible to create an MPA network that increases food. 

The relevant questions then are by how much and where? Here is where we feel this model has both errors and assumptions that produce results that are innacurate, even by the scale of a global analysis. 
...
run_trimmed_cabral_et_al <- FALSE
...
In terms of the actual calculations of effect of MPAs on harvest, there is no explicit space in the model, beyond essentially a two-patch system with a single carrying capacity, K, and movement in each patch a function of MPA size. So, this means that for any individual stock, the food maximizing MPA size is simply a function of life history and the assumption about BAU conditions: A stock with F/F~MSY~ BAU of 1.5 will need a much bigger MPA than one with  F/F~MSY~ BAU of 1, and a stock with  F/F~MSY~ BAU of 0.5 should maintain or lose catch from any MPA under these assumptions. So, without any spatial mapping, one could, taking the Costello et al. 2016 estimates of BAU stock status as given, as well as the assumptions of this model, calculate the optimal percent of each stock's K that should be in an MPA to maximize food. This two-patch model forms the core of the Cabral et al. analysis.

The concerning issues begin to arise in then trying to translate that *percent* protected onto a physical map of the world. By our understanding, the assumption for any unassessed stocks is that the stock's range is equal to that species' range from Aquamaps (minus any overlapping range with RAM stocks of the same species). Translating from the stylized surplus production (two-patch) model to the globe then requires a few critical assumptions. Namely:

1. Probability of occurrence across space (the output variable in Aquamaps) is directly proportional to biomass (K) across space
...
One could make a case for any one of these assumptions, but there are several features of the data used and the results produced that suggest to us these assumptions produce potentially misleading results.

In terms of food maximization, clearly the assumption about the connectivity of these systems matter. Take the largehead hairtail (*Trichiurus lepturus*) as an example, a stock that spans the entire globe (Fig.\@ref(fig:lht-plot)). Consider a hypothetical scenario where there are two completely separate stocks of largehead hairtail, one that lives in the Pacific and Indian Oceans, and the other in the Atlantic. Suppose then that F/Fmsy is 2 in the Pacific/Indian Ocean and F/Fmsy = 1 in the Atlantic (and assume that both have equal total K and MSY). What would be the consequences of closing off the Atlantic inside an MPA for food production of largehead hairtail? Assuming no connectivity between the Atlantic and Pacific stocks, the Pacific/Indian Ocean stock would be driven to collapse due to high fishing pressure (assuming Schaefer dynamics), and the Atlantic stock would rebuild to carrying capacity. In sum, the MPA would cause a loss in food production relative to BAU that would be equal to the MSY of the Atlantic stock.

But, what if in reality the Atlantic and Pacific are one shared stock in the manner described in Cabral et al. 2020, with an MSY weighted F/Fmsy BAU of 1.5? Closing of the Atlantic inside an MPA might well then provide substantial food benefits, to the extent that the Atlantic MPA brings the biomass-weighted mean fishing mortality rate of the total stock in line with Fmsy. In other words, you could theoretically still fish like crazy in the Pacific / Indian Ocean, but it is now being bolstered by spillover from the Atlantic MPA. Clearly  then, decisions about how to aggregate stocks play a major role in what an global network of MPAs for food would look like as we move from the stylized two-patch model to an actual map of the world. 
...
```{r lht-plot, fig.cap = "Extent of largehead hairtail (*Trichiurus lepturus*) stock in KprotectedPerCell_Library"}
tmp <- ShortCoord %>% 
...
Designing an MPA network for any objective clearly requires some knowledge of the spatial distribution of fish species/stocks, as well as the movement rates of organisms within that spatial distribution. Both of these are also poorly understood for many species, and so we appreciate the need for the simplifying assumptions made by Cabral et al. However, we feel that there are some substantial issues with treating the species distribution model (SDM) from AquaMaps as both an index of the relative biomass in space, *and* as a measure of the range of the connected biological stock.
...
As mentioned above, these methods for Aquamaps were specifically chosen to err on the side of positive bias in species presence, because they are meant (primarily) as the basis for analyses describing global patterns of marine biodiversity. However, the approach has serious implications when used to extrapolate fish abundance across space, as in the Cabral et al. analysis. The trapezoidal shape of environmental affinities is designed for maximum coverage, and will naturally result in inflated ranges of high species occurrence probability. It means, for example, that a spatial cell that is in the 10th percentile of a species’ observed tolerance for depth, temperature, and salinity will be treated exactly the same as a cell at the 50th percentile of these values. Because the Cabral et al. analysis extrapolates the Aquamaps probabilities of occurrence to abundance---by assuming that total modeled carrying capacity is distributed across the Aquamaps range, and that the relative carrying capacity assigned to each cell is directly proportional to the probability of occurrence in that cell---it follows that the abundance of each species will be distributed homogeneously across a wide swath of ocean.
...
We understand that simplifying assumptions must be made in efforts to make global assessments, and we do not know of a better global model of species distributions that encompasses such a range of species and could be used off the shelf. However, based on the structural assumptions of AquaMaps, it appears likely that using Aquamaps as an index of abundance will positively bias the assumed stock range used in Cabral et al. 2020. Moreover, this approach does not account for physical barriers to connectivity that would also reduce the effective stock size (e.g., Fig.\@ref(fig:lht-plot) would assume that growth and movement from small patches in the mid-Atlantic are just as connected to China as patches off of Japan). 
...
cs_MPAcoverage<-ShortCoord_Sort %>% filter(sign) %>%  ggplot(aes(x=lon,y=lat)) + #scale_fill_gradient2(low="darkred", mid="white",high="#00539CFF",midpoint = 0, space = "Lab",na.value = "grey50", guide = "colourbar", aesthetics = "fill",name=expression(paste(Delta, "H (MT)")))+
  ...
  # scale_fill_gradientn(colours = c("forestgreen", "white", "orange"), limits=c(0,100), values = scales::rescale(c(min(ShortCoord_Sort$rank), InflectMPA, max(ShortCoord_Sort$rank))),name="Protection sequence")+
  theme(axis.title.x = element_blank(),axis.title.y = element_blank(), panel.background = element_blank(),legend.position=c(0.63, 0.05), legend.direction = "horizontal")+ #"bottom
  geom_raster(aes(fill = dH / max(dH)))+
  # geom_raster(data=MPA_coord, aes(x=lon, y=lat),fill="cyan")+  #"#EEA47FFF"
  geom_sf(data = land_shp_moll, fill="black", lwd = 0, inherit.aes = F)+ theme(panel.grid.major = element_line(colour = 'transparent')) + 
  scale_fill_viridis_c()

# 
# compact_china_map <-ShortCoord_Sort %>% filter(sign, dH / max(dH) > .1) %>%  ggplot(aes(x=lon,y=lat)) + 
...
write_rds(fao_area_contributions, file = file.path(results_path,"china-contributions.rds"))
...
dh_outside_chinaish_plot <- fao_area_contributions %>% 
  ...
  scale_y_continuous(labels = scales::percent, expand = c(0,NA),
                     name = "% of MPA Harvest Benefits to China") + 
  scale_x_discrete(labels = c("Outside Pacific Northwest", "Inside Pacific Northwest"), name = '')
...
cumu_msy_plot <- cumu %>% 
  ...
  geom_line() + 
  scale_x_continuous(labels = scales::percent, name = "% of Patches") + 
  scale_y_continuous(labels = scales::comma, name = "Cumulative MSY")
...
p_eez_closed_map <- p_eez_closed %>%
  ...
  geom_sf(size = .1) +
  scale_color_viridis_c(labels = scales::percent, name = "% of EEZ in food increasing MPA", guide = guide_colorbar(barwidth = unit(15,"lines"))) + 
...
weighted_p_eez_closed_map <- p_eez_closed %>%
  ...
  geom_sf(size = .1) +
  scale_color_viridis_c(labels = scales::percent, name = "% of K-weighted EEZ in food increasing MPA", guide = guide_colorbar(barwidth = unit(15,"lines"))) + 
  theme(legend.position = "top", legend.direction = "horizontal")
...
p_eez_closed_plot <- p_eez_closed %>% 
  ...
  geom_col() + 
  coord_flip() + 
  scale_y_continuous(labels = scales::percent, name =  "% of EEZ in food increasing MPA") + 
...
p_eez_closed_map <- p_eez_closed %>%
  ...
  geom_sf(size = .1) +
  scale_color_viridis_c(labels = scales::percent, name = "% of EEZ in food increasing MPA", guide = guide_colorbar(barwidth = unit(15,"lines")), limits = c(0,1)) + 
  theme(legend.position = "top", legend.direction = "horizontal")
...
```{r ram-top5-protected-plot, fig.cap = "Histogram of percent of Free's estimate of RAM footprint protected inside MPAs inside the top 5% ranking of catch increasing MPAs"}
...
There F/Fmsy is pulled from RAM, and the *r* values are pulled from Cabral et al. 2020. We then re-ran the Cabral et al. 2020 analysis, but with these corrected BAU escapements for RAM stocks. We note that there are 205 RAM stocks in this analysis that have no F/Fmsy values reported in RAM, but which are included in Cabral et al. 2020. Since we are not clear what rules were used to assign BAU policies for these stocks (some of which have ERbest values in RAM, so guessing those were used) we omit them from this analysis. As such, some of these differences are due to the difference in stocks. However, this subset of RAM stocks contains 88% of the total MSY of RAM stocks reported in Cabral et al. 2020, so it is unlikely to have a dramatic effect on results. To clearly compare the effect of the BAU policies, we re-ran the Cabral et al. 2020 analysis, but removing any RAM stocks that do not have F/Fmsy values reported in RAM. We then compared the results for that same subset of stocks but using the corrected RAM BAU policy. 

Overall, using the corrected RAM BAU policy produces a lower estimate of global maximum upside from MPA protection, 6.91 MMT down from 9.97 MMT (relative to the 10.49 MMT of upside with all the RAM stocks used in Cabral et al. 2020). While this is a non-trivial difference, a 30% reduction in projected upside, and should be corrected so as not to confuse public discourse around the estimated magnitude of fishing losses under BAU, it is still in the order of magnitude as the original results from Cabral et al. 2020 (Fig.\@ref(fig:cor-ben-plot)). 
...
<!-- While this correction is easy to apply and would keep the results in line what what might be considered a reasonable BAU policy for RAM stocks, given the other issues raised in this summary we do not suggest that this corrected map represents the "right" value.  -->

```{r run-trimmed-cabral, include=FALSE}
#Load MOLLWEIDE projected data
MegaData<-readRDS(file = here("data","MegaData.rds"))
Cleanmegacell<-readRDS(file = here("data","Cleanmegacell_mollweide.rds"))
CleanCoordmegacell<-readRDS(file = here("data","CleanCoordmegacell_mollweide.rds"))
...
has_match <- !(MegaData$Manage == 1 & is.na(MegaData$ram_escapement_bau))

KprotectedPerCell_Library <- KprotectedPerCell_Library[has_match,]

MegaData <- MegaData %>% 
  filter(!(Manage == 1 & is.na(ram_escapement_bau)))
...
if(scenario=="BAU1"){
  saveRDS(NetworkResult,file = file.path(results_path,"trimmed_NetworkResult100_BAU1_mollweide.rds"))
  saveRDS(PriorityAreas,file = file.path(results_path, "trimmed_PriorityAreas100_BAU1_mollweide.rds"))
}else if(scenario=="all managed"){  
  saveRDS(NetworkResult,file = "~/foodGCEfile/NetworkResult100_allmanaged_redistribute.rds")
  saveRDS(PriorityAreas,file = "~/foodGCEfile/PriorityAreas100_allmanaged_redistribute.rds") 
}else if(scenario=="OAconstant"){
  ...
  NetworkResult <-readRDS(file = file.path(results_path,"trimmed_NetworkResult100_BAU1_mollweide.rds"))
  
  PriorityAreas <- readRDS(file = file.path(results_path,"trimmed_PriorityAreas100_BAU1_mollweide.rds") )
...
BenefitCurve<-rbind(BenefitCurve,zerozero)
theme_set(theme_cowplot())
trimmed_benefitplot<-ggplot(BenefitCurve, aes(MPA, NetworkResult)) +geom_line(col="blue")+# theme_classic()+
  labs(x="% EEZ protected",y="Change in catch (MMT)",title=paste("Global (max =", round(max(BenefitCurve$NetworkResult),2),"MMT)"))#+geom_hline(yintercept = 0)
trimmed_benefitplot
...
trimmed_MPAcoverage<-ShortCoord_Sort %>% ggplot(aes(x=lon,y=lat,fill=rank)) + #scale_fill_gradient2(low="darkred", mid="white",high="#00539CFF",midpoint = 0, space = "Lab",na.value = "grey50", guide = "colourbar", aesthetics = "fill",name=expression(paste(Delta, "H (MT)")))+
  scale_fill_gradientn(colours = c("forestgreen", "white", "orange"), limits=c(0,100), values = scales::rescale(c(min(ShortCoord_Sort$rank), InflectMPA, max(ShortCoord_Sort$rank))),name="Protection sequence")+
  theme(axis.title.x = element_blank(),axis.title.y = element_blank(), panel.background = element_blank(),legend.position=c(0.63, 0.05), legend.direction = "horizontal")+ #"bottom
  geom_raster()+
  geom_raster(data=MPA_coord, aes(x=lon, y=lat),fill="cyan")+  #"#EEA47FFF"
...
trimmed_p_eez_closed <- spatial_msy %>%
  group_by(iso3c_name, iso3_code) %>%
  summarise(
    p_closed = mean(d_h > 0),
    weighed_p_closed = weighted.mean(d_h > 0, w = msy_per_cell),
...
trimmed_p_eez_closed_map <- trimmed_p_eez_closed %>%
  ggplot(aes(color = p_closed)) +
  geom_sf(
    data = land_shp_moll,
    fill = "black",
...
```{r run-corrected-cabral, include=FALSE}
...
(trimmed_benefitplot + labs(subtitle = "A) Trimmed Cabral BAU")) / (corrected_benefitplot + labs(subtitle = "B) Corrected RAM BAU"))
```
...
compact_corrected_p_eez_closed_map <- p_eez_closed %>%
  ggplot(aes(color = p_closed)) +
  geom_sf(
    data = land_shp_moll,
    fill = "black",
  ...
  geom_sf(size = .1) +
  scale_color_viridis_c(labels = percent, name = "% of EEZ in food increasing MPA", limits = c(0,1)) + 
  theme(legend.position = "top", legend.direction = "horizontal")
...
corrected_comparison_p_eez_closed_plot <- p_eez_closed %>% 
  # filter(cumu_capture < 60e6) %>% 
  filter(capture_rank > 0.9) %>% 
  as_tibble %>% 
  select(iso3c_name,top_p_closed,capture,p_closed) %>% 
  ungroup() %>% 
  pivot_longer(contains("_closed"), names_to = "tmp", values_to = "percent_closed") %>% 
  mutate(tmp = case_when(tmp == "p_closed" ~ "All Food Increasing", TRUE ~ "Top 5% Food Increasing"))  %>% 
  ggplot(aes(reorder(iso3c_name,percent_closed), percent_closed, fill = tmp)) + 
...
```{r cor-map, fig.cap="Percent of each country's EEZ that could be placed in global food-increasing MPA from using Cabral et al. 2020 BAU policy for RAM stocks (A) and using the corrected RAM BAU policy (B). Both cases use the same subset of unassessed and RAM stocks."}
(trimmed_p_eez_closed_map + labs(subtitle ="A) Trimmed Cabral BAU")) / (corrected_p_eez_closed_map + labs(subtitle = "B) Trimmed Corrected BAU") + guides(color = FALSE))
```
...
## top panel: corrected peez with contributions to china
...
eezs <-
  ...
  mutate(iso3_code = countrycode(SOVEREIGN1, "country.name",  "iso3c")) %>% 
    mutate(iso3c_name = countrycode(iso3_code, "iso3c",  "country.name")) %>%
  filter(!is.na(iso3_code)) %>%
  sf::st_transform(sf::st_crs(fao_areas))
...
fao_area_center <- fao_areas %>% 
       recenter_vector(center = -200) %>%
  # sf::st_shift_longitude() %>%
  sf::st_centroid()


fao_area_center_coords <- fao_area_center %>% 
  sf::st_coordinates()

fao_area_contributions <- read_rds(file = file.path(results_path,"china-contributions.rds")) %>% 
  as_tibble() %>% 
  select(fao_area_code, percent_delta_h)


test <- fao_area_center %>% 
  cbind(fao_area_center_coords)

pne <- test %>% 
  filter(fao_area_code == 61) %>% 
  as_tibble() %>% 
  select(X,Y) %>% 
...
not_pne <- test %>% 
  filter(fao_area_code != 61) %>% 
  as_tibble() %>% 
  select(fao_area_code,X,Y) %>% 
  cbind(pne) %>% 
  left_join(fao_area_contributions, by = "fao_area_code") %>% 
  filter(percent_delta_h > 0.05) %>%
  mutate(contrib_label = percent(percent_delta_h, accuracy = 1))


land_sf <- rnaturalearth::ne_countries(scale = 110, returnclass = 'sf')

land_recenter <- recenter_vector(land_sf,center = -200)


reply_a_fig <- trimmed_p_eez_closed %>% 
  sf::st_transform(crs = sf::st_crs(fao_areas)) %>% 
     recenter_vector(center = -200) %>%
  ggplot() +
    # geom_sf(data = fao_areas, fill = "transparent") +
  geom_sf(aes(color = p_closed, fill = p_closed), size = .1) +
  geom_sf(
    data = land_recenter,
    fill = "darkgrey",
    lwd = 0,
  ...
  geom_segment(data = not_pne,aes(x = X, y = Y,xend =x_end, yend = y_end),alpha = 1, linetype = 2,
               color = "red") +
  geom_label(data = not_pne, aes(x = X, y = Y, label = contrib_label), alpha = 0.75, size = 4) +
  scale_color_viridis_c(labels = percent, name = "% in Food Increasing MPA", guide = guide_colorbar(barheight = unit(7,"lines"),
                                                                                 frame.colour = "black"), limits = c(0,1)) + 
  scale_fill_viridis_c(labels = percent, name = "% in Food Increasing MPA", guide = guide_colorbar(barheight = unit(7,"lines"),
                                                                                                 frame.color = "black"), limits = c(0,1)) +
    theme(
...
#         legend.margin =  margin(0, 14, 2, 12),
#         axis.title = element_blank())
reply_a_fig

```
## bottom panel
Now do the australia example

```{r}

cabral_mpa <-  cabral_mpa %>% 
  sf::st_transform(crs = sf::st_crs(fao_areas)) 
...
reply_b_fig <-cabral_cells %>%
  sf::st_transform(crs = sf::st_crs(fao_areas)) %>% 
   recenter_vector(center = -200) %>% 
  # sf::st_shift_longitude() %>% 
  filter(scaled_benefits > 0.01) %>% 
  ggplot() + 
    geom_sf(
    data =land_recenter,
    fill = "darkgrey",
  ...
  geom_sf(aes(color=scaled_benefits), size = .1) +
  geom_sf(data = cabral_mpa %>% filter(mpa) %>% recenter_vector(center = -200), color = "red", size = .1) +
  scale_color_viridis_c(name = "Benefits / max(Benefits)", guide = guide_colorbar(barheight = unit(7,"lines"),
                                                                  frame.colour = "black"),
                        option = "A") + 
  theme(
        plot.margin = margin(0,10,0,1))
...
tmp <- foot_dist_plot +theme(plot.margin = margin(0,0,0,0)) + 
  theme(axis.title = element_text(size = 6),
        plot.background = element_rect(fill = "red"))
reply_plot<-
 ((((
    reply_b_fig + labs(subtitle = "A")) / (reply_a_fig + labs(subtitle = "B"))))) & theme(
    axis.text = element_text(size = 6),
    legend.title = element_text(size = 10),
    legend.text = element_text(size = 8),
    plot.margin = unit(c(.1, -10, -.1, -10), "lines")
  )

reply_plot

write_rds(reply_plot, file = file.path(results_path,"reply_plot.rds"))


ggsave(reply_plot,filename = file.path(results_path,"reply-plot.png"),
       height = 8,
       width = 8)
...
reply_plot<-
 (tmp |(((
    reply_a_fig + labs(subtitle = "a")) / (reply_b_fig + labs(subtitle = "b"))))) + plot_layout(ncol = 2, widths = c(1,3)) & theme(
    axis.text = element_text(size = 6),
    legend.title = element_text(size = 6),
    legend.text = element_text(size = 6),
    plot.margin = unit(c(-.075, -10, -.1, -10), "lines")
  )
...
UPsides summary 
```{r}
usa_v_world <- upsides_bau %>% 
  filter(country %in% c("USA",'China',"Indonesia","India"))


upsides_summary <- usa_v_world %>% 
  group_by(country) %>% 
  summarise(upside_potential = sum(msy - catch),
            median_fbau = median(fvfmsy_upsides),
            mean_fbau = mean(fvfmsy_upsides))


comparison <- trimmed_p_eez_closed %>% 
  filter(iso3c_name %in% c("United States","China","Indonesia","India"))
  

upsides_summary <- usa_v_world %>% 
  filter(country %in% c("USA","China","Indonesia","India")) %>% 
  mutate(usa = country == "USA") %>% 
  group_by(usa) %>% 
  summarise(upside_potential = sum(msy - catch),
            median_fbau = median(fvfmsy_upsides),
            mean_fbau = mean(fvfmsy_upsides))

f_comparison <- round(upsides_summary$median_fbau[upsides_summary$usa == FALSE] / upsides_summary$median_fbau[upsides_summary$usa == TRUE],1)

catch_comparison <- round(upsides_summary$upside_potential[upsides_summary$usa == FALSE] / upsides_summary$upside_potential[upsides_summary$usa == TRUE],1)

write_rds(upsides_summary, file = file.path(results_path,"upsides_summary.rds"))
...
```{r}
  rmarkdown::render(
    here::here("cabral-etal-reply.Rmd"),
