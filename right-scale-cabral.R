...
R<-rowSums(KprotectedPerCell_Library[,which(MPAselect==1),drop=FALSE], na.rm = TRUE)
...
if (run_cabral_et_al){
  ...
  if(scenario=="BAU1"){
    saveRDS(NetworkResult,file = file.path(results_path,"downsized_NetworkResult100_BAU1_mollweide.rds"))
    saveRDS(PriorityAreas,file = file.path(results_path, "downsized_PriorityAreas100_BAU1_mollweide.rds"))
  ...
  NetworkResult <-readRDS(file = file.path(results_path,"downsized_NetworkResult100_BAU1_mollweide.rds"))
  
  PriorityAreas <- readRDS(file = file.path(results_path,"downsized_PriorityAreas100_BAU1_mollweide.rds") )
...
# do it again but with RAM correction -------------------------------------
...
treacherous_trachurus <- (str_detect(divided_megadata$SciName,"Trachurus trachurus"))

divided_megadata <- divided_megadata %>% 
  filter(!treacherous_trachurus)
  
divided_k <- divided_k[!treacherous_trachurus,]
...
if (run_corrected_cabral_et_al){
  ...
  if(scenario=="BAU1"){
    saveRDS(
      ...
      file = file.path(
        results_path,
        "downscaled_corrected_NetworkResult100_BAU1_mollweide.rds"
      )
    )
    saveRDS(
      PriorityAreas,
      file = file.path(
        results_path,
        "downscaled_corrected_PriorityAreas100_BAU1_mollweide.rds"
      )
    )
  ...
  NetworkResult <-readRDS(file = file.path(results_path,"downscaled_corrected_NetworkResult100_BAU1_mollweide.rds"))
  
  PriorityAreas <- readRDS(file = file.path(results_path,"downscaled_corrected_PriorityAreas100_BAU1_mollweide.rds") )
...
corrected_MPAcoverage<-ShortCoord_Sort %>% ggplot(aes(x=lon,y=lat,fill=rank)) + #scale_fill_gradient2(low="darkred", mid="white",high="#00539CFF",midpoint = 0, space = "Lab",na.value = "grey50", guide = "colourbar", aesthetics = "fill",name=expression(paste(Delta, "H (MT)")))+
  scale_fill_gradientn(colours = c("forestgreen", "white", "orange"), limits=c(0,100), values = scales::rescale(c(min(ShortCoord_Sort$rank), InflectMPA, max(ShortCoord_Sort$rank))),name="Protection sequence")+
  theme(axis.title.x = element_blank(),axis.title.y = element_blank(), panel.background = element_blank(),legend.position=c(0.63, 0.05), legend.direction = "horizontal")+ #"bottom
  geom_tile()+
  geom_raster(data=MPA_coord, aes(x=lon, y=lat),fill="cyan")+  #"#EEA47FFF"
...
(benefitplot + labs(subtitle = "Global Stocks") ) / 
  (downsized_benefitplot + labs(subtitle = "Taxa + EEZ + FAO Region Stocks")) / 
  (corrected_benefitplot + labs(subtitle = "Corrected Data + Taxa + EEZ + FAO Region Stocks"))



(MPAcoverage + labs(title = "Cell Prioritization Rank with Global Stocks") ) / 
  (downsized_MPAcoverage + labs(title = "Cell Prioritization Rank with Taxa + EEZ + FAO Region Stocks")) / 
  (corrected_MPAcoverage + labs(title = "Cell Prioritization Rank with Corrected Data + Taxa + EEZ + FAO Region Stocks")) 
