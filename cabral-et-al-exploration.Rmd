...
The idea: pick one region / country, and demonstrate how far the benefits from that MPA are dispersed


1. Pick a place (let's say Chile). 

2. Pick out any species that has some overlap with that areas in its range

3. run MPA optimization but only for the cells in chile

4. Store total upsides... wait
  - if you have total upsides from that MPA, the question then is where do those upsides go. Suppose that 99% of the upsides went to one species, you don't wannt to attribute it on the map to another. 
  - You need to store upside by species for each cell protected. 
  - Then,  distribute benefit by species acacording to their distribution
  - Then, sum benefits by cell
...
teleport_map <-cabral_cells %>% filter(scaled_benefits > 0.1) %>% ggplot() + 
  ...
  geom_raster(aes(x = X, y = Y,fill=scaled_benefits),alpha = 0.5)+
  geom_raster(data = cabral_mpa %>% filter(mpa),aes(x = X, y = Y), color = "red") +
  geom_sf(data = land_shp_moll, fill="black", lwd = 0, inherit.aes = F)+ theme(panel.grid.major = element_line(colour = 'transparent')) + 
