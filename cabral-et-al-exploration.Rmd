...
warning("Each 'run_` takes about 10 hours on 10 cores, be prepared to wait if running everything")

rename <- dplyr::rename

run_cabral_et_al <- FALSE

run_trimmed_cabral_et_al <- TRUE
...
Overall, as suspected, using the corrected RAM BAU policy produces a lower estimate of global maximum upside from MPA protection, 6.91 MMT down from 10.49 MMT. While this is a non-trivial difference, and should be corrected so as not to confuse public discourse around the estimated magnitude of fishing losses under BAU (and unclear how much of this is simply due to the omitted RAM stocks, though given that most RAM stocks are in relatively good shape, most of the upsides shouldn't be coming from them anyway), it is still in the order of magnitude as the original results from Cabral et al. 2020 (Fig.\@ref(fig:cor-ben-plot)). 

Spatially though we see come clearer differences. Given the largely coastal nature of RAM, we restrict our spatial analysis to percent coverage within EEZ. Particular for countries with large, stock-assessed fisheries (e.g. those in South America and the USA), the percentage of that country's EEZ that the Cabral et al. 2020 model states could be placed in a food-increasing MPA decreases substantially, while increasing in other areas (e.g. the Mediterranean) (Fig.\@ref(fig:cor-map)). Again though, we would need to consider these differences in light of the slightly different stock mix used here, due to lack fo F/Fmsy values for many RAM stocks included here. 
...
trimmed_p_eez_closed_map <- p_eez_closed %>%
  ggplot(aes(color = p_closed)) +
  geom_sf(
    data = land_shp_moll,
    fill = "black",
...
(benefitplot + labs(subtitle = "A) Cabral BAU")) / (corrected_benefitplot + labs(subtitle = "B) Corrected RAM BAU")) / (trimmed_benefitplot + labs(subtitle = "C) Trimmed Cabral BAU"))
```
...
(p_eez_closed_map + labs(subtitle ="A) Cabral BAU")) / (corrected_p_eez_closed_map + labs(subtitle = "B) Corrected BAU") + guides(color = FALSE))  / (trimmed_p_eez_closed_map +
 labs(subtitle = "C) Trimmed Corrected BAU") + guides(color = FALSE))
```
